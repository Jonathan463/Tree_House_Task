...
import com.treehouseapp.repository.*;
...
public class CheckoutServiceImpl implements CheckoutService {
    ...
    public CheckoutResponse createOrderFromCartItem(long userId, long cartId) {
        ...
        Optional<Cart> cart = cartRepository.getByUser_Id(userId);
        Optional<Order> myorder = orderRepository.findByUserIdAndStatus(userId, OrderStatus.PENDING);
        if (myorder.isPresent()){
//            orderDetailsRepository.deleteAllByOrder_Id(myorder.get().getId());
            List<OrderDetail> orderDetailList = listOfCartItems.stream().map(cartItem -> {
                ...
                orderDetail.setOrder(myorder.get());
            ...
            return  new CheckoutResponse(myorder.get().getId(), list, userShippingAddress);
        }
        ...
        return  new CheckoutResponse(order.getId(), list, userShippingAddress);
    }
