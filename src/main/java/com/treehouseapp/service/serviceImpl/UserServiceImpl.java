...
import com.treehouseapp.dto.*;
import com.treehouseapp.model.token.ConfirmationToken;
import com.treehouseapp.payload.UpdatePayLoad;
import com.treehouseapp.security.PasswordValidator;
import com.treehouseapp.security.service.UserDetailsImpl;
...
import com.treehouseapp.payload.JwtResponse;
    ...
    public ResponseEntity<?> loginUser(@RequestBody UserDto loginRequest, HttpServletResponse response) throws Exception {
        try {
            ...
            if(user.getEnabled()) {
                ...
                if(loginRequest.getCartItems() == null){
                    return ResponseEntity.ok(new JwtResponse(jwt, userDetails.getUsername(), user.getId(), roles));
                }
                loginRequest.getCartItems().forEach((cartItem)->
                        cartService.addToCart(new CartDTO(cartItem.getProductQuantity()), user.getId(), cartItem.getProductId()));
                return ResponseEntity.ok(new JwtResponse(jwt, userDetails.getUsername(), user.getId(), roles));
